...
// Step 1's placeholder (see the Commerce plan) upgraded to a real Overview — stat tiles
// and the orders list are honest about being empty until checkout (step 2) exists; this
// page starts reading real data on day one rather than shipping fake numbers now and
...
export default async function CommerceOverviewPage() {
  ...
  const [productCount, orderCount, salesTotal, upcomingBookingCount, lowStockCount, recentOrders] = await Promise.all([
    prisma.commerceProduct.count({ where: { organizationId, active: true } }),
    prisma.commerceOrder.count({ where: { organizationId, status: "PAID" } }),
    prisma.commerceOrder.aggregate({ where: { organizationId, status: "PAID" }, _sum: { amountMinor: true } }),
    prisma.commerceBooking.count({
      where: { organizationId, status: "CONFIRMED", scheduledStart: { gte: new Date() } },
    }),
    prisma.commerceProduct.count({
      where: { organizationId, active: true, kind: "PHYSICAL", stockQuantity: { not: null, lte: 5 } },
    }),
