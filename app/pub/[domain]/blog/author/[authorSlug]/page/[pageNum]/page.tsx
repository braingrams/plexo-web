...
type Params = { domain: string; authorSlug: string; pageNum: string };
...
export default async function AuthorArchivePaginatedPage({ params }: { params: Promise<Params> }) {
  const { domain, authorSlug, pageNum } = await params;
  const page = Number.parseInt(pageNum, 10);
  if (!Number.isInteger(page) || page < 2) notFound();
  const site = await resolveBlogSite(decodeURIComponent(domain));
  return <BlogArchivePage domain={domain} site={site} kind="author" slug={authorSlug} page={page} />;
}
